...
export default {
  ...
  created() {
    this.recentActivities({ user_id: urlSafeBase64Decode(this.$route.params.userEmail) })
    this.certificateInfo({ user_id: urlSafeBase64Decode(this.$route.params.userEmail) })
  },
  data() {
    return {
      emailInfo: urlSafeBase64Decode(this.$route.params.userEmail),
      username: urlSafeBase64Decode(this.$route.params.userName),
      weeklyMonthlyTopData: 'monthly',
      showOverallWeeklyMonthly: false,
      reportWindowStatusTopData: false
    }
  },
  methods: {
    ...mapActions('users', ['recentActivities', 'certificateInfo']),
    ...mapActions('dashboard', {
      monthlyWeeklyreportOverallStatsIndividualUser: 'monthlyWeeklyreportOverallStatsIndividualUser'
    }),
    async toggleInfoTopCourses(val) {
      if (this.weeklyMonthlyTopData === 'weekly' || this.weeklyMonthlyTopData === 'monthly') {
        this.showOverallWeeklyMonthly = true
      } else {
        this.showOverallWeeklyMonthly = false
      }
    },
    reportWindowTopData() {
      if (this.reportWindowStatusTopData) {
        this.reportWindowStatusTopData = false
      } else {
        this.reportWindowStatusTopData = true
      }
    },
    onCancelWindowTopData() {
      this.reportWindowStatusTopData = false
    },
    async downloadableReportTopData(type) {
      await this.monthlyWeeklyreportOverallStatsIndividualUser({
        user_id: urlSafeBase64Decode(this.$route.params.userEmail),
        period: this.weeklyMonthlyTopData
      })
      const requiredFields = {
        Courses: this.weeklyMonthlyJsonIndividualUserGetter.data.eveValuesReport,
        Labs: this.weeklyMonthlyJsonIndividualUserGetter.data.LabsValuesReport,
        'Learning paths': this.weeklyMonthlyJsonIndividualUserGetter.data.lpValuesReport
      }
      if (type === 'json') {
        const dataStr = 'data:text/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(requiredFields))
        const dlAnchorElem = document.getElementById('downloadAnchorElem')
        dlAnchorElem.setAttribute('href', dataStr)
        dlAnchorElem.setAttribute('download', `${this.capitalizeFirstLetter(this.weeklyMonthlyTopData)}-courses-labs-lp-report.json`)
        dlAnchorElem.click()
      }
      if (type === 'csv') {
        const headers = {}
  ...
  computed: {
    ...
    ...mapGetters('dashboard', {
      weeklyMonthlyJsonIndividualUserGetter: 'weeklyMonthlyJsonIndividualUserGetter'
    })
  }
}
</script>
