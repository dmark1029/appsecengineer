<template>
  ...
  >
    <q-table
      ...
      :columns="columns"
      dark
      :rows="deliveryStore.fetchlistDelivery"
      hide-bottom
      row-key="id"
    ...
    >
      ...
      <template v-slot:bottom-row>
        <q-tr v-if="deliveryStore.fetchlistDelivery.length < deliveryStore.totalDeliveries">
...
async function loadMoreDeliveries() {
  const data = {
    pagination: {
      length: 80,
      startIndex: deliveryStore.fetchlistDelivery.length,
