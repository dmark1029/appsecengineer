...
const emit = defineEmits(['reportDetail'])
...
export default {
  ...
  methods: {
    ...
    onReset() {
      this.filter = ''
      this.search = ''
      this.certificationReport({ test: this.reportData.sk })
    },

    onFilter() {
      const data = { test: this.reportData.sk }
      if (this.search) {
        data.query = this.search
      }
      const filterType = this.filter?.split(':')
      if (this.filter && filterType[0] === 'status') {
        data.status = filterType[1]
      }
      if (this.filter && filterType[0] === 'certified') {
        data.certified = JSON.parse(filterType[1])
      }
