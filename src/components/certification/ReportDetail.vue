  ...
  <q-card dark flat>
    <q-card-section>
      <div class="flex items-center justify-between">
        <div class="text-subtitle1">
          ...
          Report Detail ({{ this.reportData.first_name }} {{ this.reportData.last_name }})
        </div>
      </div>
    </q-card-section>
    <q-card-section class="q-pt-none row" style="display: flex; align-items: normal; justify-content: center">
      <q-card dark class="detail-card">
        ...
        <q-btn
          v-if="this.reportData.attachment_url"
          class="download-attachment"
          color="positive"
          label="Download attachment file"
          no-caps
          size="12px"
          @click="downloadFile(this.reportData.attachment_url)"
        />
      </q-card>
      <q-card dark class="detail-card">
...
import { max, min } from 'src/utils/rules'
...
export default {
  ...
  mounted() {
    this.challenges = this.reportData.challenges
    this.videoUrl = this.reportData.meet
    this.reportRow = this.reportData
    this.questions = this.reportData.questions
    this.resultsBeforeChange = this.reportData.questions.map((question) => question.ai_eval_result)
    this.tableData = [
      {
        index: 1,
        label: 'Email',
        value: this.reportData.email
      },
      {
        index: 2,
        label: 'First Name',
        value: this.reportData.first_name
      },
      {
        index: 3,
        label: 'Last Name',
        value: this.reportData.last_name
      },
      {
        index: 4,
        label: 'Date',
      ...
      },
      {
        index: 5,
        label: 'Status',
        value: this.reportData.status
      },
      {
        index: 6,
        label: 'Scored',
        value: this.reportData.score
      },
      {
        index: 7,
        label: 'Total Score',
        value: this.reportData.total_score ? this.reportData.total_score : 0
      },
      {
        index: 8,
        label: 'Percentage',
        value: this.reportData.percentage ? this.reportData.percentage : 0
      },
      {
        index: 9,
        label: 'Certified',
        value: this.reportData.certified ? 'Yes' : 'No'
      }
    ]
  },
  methods: {
    ...mapActions('certification', ['certificationIssue', 'certificationReport', 'updateScores']),
    ...mapMutations('certification', ['UPDATE_STATE_SCORES']),
    ...mapMutations('certification', ['LOADING']),

    onChallengesDialog(row) {
      this.challengesDialog = true
      this.challenges = row.challenges
    },
    showEmail(row) {
      this.detailDialog = true
      this.$emit('reportDetail', { show: true, row })
    },
    async onVideoDialog(row) {
      this.showVideo = true
      this.videoUrl = row.meet
      if (this.$refs.videoPlayer) {
        this.$refs.videoPlayer.play()
      }
    },
    onQuestionsDialog(row) {
      this.questionsDialog = true
      this.reportRow = row
      this.questions = row.questions
      this.resultsBeforeChange = row.questions.map((question) => question.ai_eval_result)
    },

    onIssueCertificate(row) {
    ...
    async issueCertificate() {
      const data = {
        ...
        test: this.reportData.sk
      }
      await this.certificationIssue(data).then(() => {
        this.issueCertificateDialog = false
      })
    },

    updateStateScores(question, label, value) {
      const payload = {
        question: question.id,
        email: this.reportRow.email,
        result: {
          ...question.ai_eval_result,
          [label]: parseInt(value)
        }
      }
      this.UPDATE_STATE_SCORES(payload)
    },

    onUpdateScores(question) {
      const payload = {
        question: question.id,
        email: this.reportRow.email,
        test: this.reportData.pk,
        result: question.ai_eval_result
      }
      this.updateScores(payload).then(() => {
        this.resultsBeforeChange = this.questions.map((question) => question.ai_eval_result)
      })
    },
  ...
  computed: {
    ...mapState('certification', ['reports']),
    formattedDate() {
      const date = new Date(this.reportData.created_on)
      const year = date.getFullYear()
      const month = String(date.getMonth() + 1).padStart(2, '0')
      const day = String(date.getDate()).padStart(2, '0')
      return `${year}-${month}-${day}`
