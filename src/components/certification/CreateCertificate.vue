      ...
      <q-card-section class="q-pa-sm">
        <q-form greedy ref="formRef" @submit="onSubmit">
          <q-stepper active-color="white" alternative-labels animated dark done-color="positive" header-nav ref="stepper" v-model="step">
            ...
            <q-step :name="2" title="Project Info" icon="more_vert" :done="step > 2" :header-nav="step > 2">
              <q-item class="col-12">
                <q-item-section>
                  <BaseInput
                    ...
                    :rules="[...minLength(20)]"
                    type="textarea"
                    v-model="certification.project_detail"
                  />
                  <p v-if="fetchErrorMsgs.project_detail" class="text-caption text-negative">
                    {{ fetchErrorMsgs.project_detail }}
                  </p>
                </q-item-section>
              </q-item>
            </q-step>
            <q-step :name="3" title="Achievement" icon="emoji_events" :done="step > 3" :header-nav="step > 3">
              <Achievement
            ...
            <template v-slot:navigation>
              <q-stepper-navigation class="row">
                ...
                <q-btn v-if="step === 3" class="col-grow" color="positive" :disable="!certification.delivery_id" @click="onSubmit()">
                  Save
                </q-btn>
...
onMounted(async () => {
  ...
  if (props.id) {
    ...
    certification.value = {
      ...
      project_detail: urlSafeBase64Decode(cert.project_detail),
...
async function onSubmit() {
  const data = {
    ...
    project_detail: urlSafeBase64Encode(certification.value.project_detail),
...
import BaseToggle from 'components/shared/BaseToggle.vue'
import { urlSafeBase64Decode, urlSafeBase64Encode } from 'src/utils/reuseFunctions'
import { fileSize, max, maxLength, min, minLength, required } from 'src/utils/rules'
import { compareFunction } from 'src/utils/validateSeralizer'
import { mapActions, mapGetters, mapState } from 'vuex'

export default {
  name: 'CreateCertification',
  components: { Achievement, BaseInput, BaseSelect, BaseToggle },
  props: ['id', 'show', 'title'],
  data() {
    return {
  ...
  async created() {
    if (!this.learningPathOption.length) {
      await this.fetchLearningPathOptions()
    }
    if (!this.chargesOptions.length) {
      await this.fetchCharges()
    }
    if (!this.skillsOptionsGetter.length) {
      this.fetchSkills({})
    }
    if (this.id) {
      const certification = this.listCertification.find((data) => data.sk === this.id)
      this.certification = {
        active: certification.active,
        challenges: certification.challenges,
        charges: certification.charges?.map((charge) => this.chargesOptions.find((data) => data.value === charge)) || [],
        delivery_id: certification.delivery_id,
        description: certification.description,
        project_detail: urlSafeBase64Decode(certification.project_detail),
        questions_count: certification.questions_count,
        duration: certification.duration,
        expiry_date: certification.expiry_date,
        issue_date: certification.issue_date,
        learningPaths: this.learningPathOption?.filter((data) => certification.learning_path_ids?.includes(data.value)),
        name: certification.name,
        pass_percentage: certification.pass_percentage,
        persona: certification.persona.map((data) => ({ label: data, value: data })),
        skills: certification.skills.map((data) => ({ label: data, value: data }))
      }
    }
  },
  methods: {
    ...mapActions('certification', ['createCertification', 'updateCertification', 'fetchCharges', 'fetchSkills']),
    ...mapActions('learningPath', ['fetchLearningPathOptions']),
    addAchievementType(achievementType) {
      this.achievementType = achievementType
    },
    addDeliveryID(event) {
      this.certification.delivery_id = event || undefined
    },
    updateDates(event) {
      if (event.dateIssued) {
        this.certification.issue_date = event.dateIssued
      }
      if (event.dateExpired) {
        this.certification.expiry_date = event.dateExpired
      }
    },
    async onSubmit() {
      const data = {
        active: this.certification.active,
        challenges: this.certification.challenges,
        charges: this.certification.charges.map((data) => data.value),
        delivery_id: this.certification.delivery_id?.value,
        description: this.certification.description,
        project_detail: urlSafeBase64Encode(this.certification.project_detail),
        questions_count: this.certification.questions_count,
        duration: this.certification.duration,
        learning_path_ids: this.certification.learningPaths.map((i) => i.value),
        name: this.certification.name,
        pass_percentage: this.certification.pass_percentage,
        persona: this.certification.persona.map((data) => data.value)
      }
      if (this.certification.skills ?? false) {
        data.skills = this.certification.skills.map((data) => data.value)
      }
      if (this.certification.expiry_date?.trim()) {
        data.expiry_date = this.certification.expiry_date.replaceAll('/', '-')
      }
      if (this.certification.issue_date?.trim()) {
        data.issue_date = this.certification.issue_date.replaceAll('/', '-')
      }
      if (this.id) {
        data.certification = this.id
        await this.updateCertification(compareFunction('certification', data, this.certification))
      } else {
        await this.createCertification(data)
      }
  ...
  computed: {
    ...mapState({
      fetchErrorMsgs: (state) => state.certification.error_msgs,
      fetchStatusOfApi: (state) => state.certification.statusOfApi,
      listCertification: (state) => state.certification.listCertification,
      chargesOptions: (state) => state.certification.chargesOptions,
      skillsOptionsGetter: (state) => state.certification.skillsOptions
    }),
    ...mapGetters('learningPath', ['learningPathOption'])
  }
}
</script> -->
