...
export default {
  ...
  async created() {
    ...
    this.topUsersData = this.topUsers.map(({ email, total_scored }) => ({ label: email, value: total_scored }))

    this.topData = this.top.map(({ certification, certification_pass_count }) => ({
      label: certification.name,
      value: certification_pass_count
    }))
  },
  methods: {
    ...mapActions('certification', ['certificationTop', 'certificationTopUsers'])
  },
  computed: {
    ...mapState('certification', ['top', 'topUsers'])
  }
}
</script> -->
