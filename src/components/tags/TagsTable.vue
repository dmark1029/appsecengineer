...
const tags = computed(() => {
  const data = store.getters['tags/fetchTagList']
  return data.filter((item) => item.tag_name.toLowerCase().includes(search.value.toLowerCase()))
})
...
function handleDelete() {
  store.dispatch('tags/deleteTag', { tag_id: selected.value.sk })
  closeDeleteModal()
}
