...
const fetchErrorMsgs = computed(() => learningPathStore.fetchErrorMsgs)
const fetchStatusOfApi = computed(() => learningPathStore.fetchStatusOfApi)
const singleLearningPathInfo = computed(() => learningPathStore.singleLearningPathInfo)
const learningPathList = computed(() => learningPathStore.learningPathList)

onMounted(async () => {
  if (props.id) {
    const learningPath = learningPathList.value.find((item) => item.id === props.id)
    learningPath.value = {
      name: learningPath.name,
      description: learningPath.description
    }
    imageData.value = learningPath.value.logo ? learningPath.value.logo : null
  }
})
const onClear = () => {
  imageData.value = ''
...
const onSubmit = async () => {
  ...
  if (props.id) {
    data.learning_path_id = props.id
